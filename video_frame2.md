# Stage 0 – Base Copy from video\_frame/page.tsx
```tsx
"use client";
...
# Stage 1 – Video-led Sync Experiment
```tsx
"use client";
...
export default function VideoFrame2Page() {
  ...
  const seekToTimeline = useCallback(
    (targetTimelineSeconds: number, autoplay = false) => {
      ...
      if (Number.isFinite(videoTime)) {
        try {
          video.currentTime = Math.max(0, videoTime as number);
        } catch {
          /* ignore */
        }
      }
      ...
      const wantsPlayback = autoplay || !video.paused;
      shouldPlayAudioRef.current = wantsPlayback;
      if (autoplay) {
        video
          .play()
          .catch(() => {
      ...
      if (desiredAudio <= 0) {
        pendingAudioPlayRef.current = wantsPlayback;
        audioStartedRef.current = false;
        setAudioPending(wantsPlayback);
        setAudioTime(0);
        try {
          audio.pause();
        } catch {
      ...
      const clampedAudio = Math.max(0, Math.min(desiredAudio, audioCap ?? desiredAudio));
      const adjustThreshold = audioStartedRef.current ? 0.25 : 0.05;
      if (Math.abs(audio.currentTime - clampedAudio) > adjustThreshold) {
        try {
          audio.currentTime = clampedAudio;
        } catch {
          /* ignore */
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      setAudioTime(Math.max(0, audio.currentTime));
    };
    const handlePlay = () => {
      audioStartedRef.current = true;
      pendingAudioPlayRef.current = false;
    ...
    const handlePause = () => {
      if (audio.currentTime <= 0.01) {
        audioStartedRef.current = false;
        pendingAudioPlayRef.current = shouldPlayAudioRef.current;
  ...
  const activeSegment = useMemo(() => {
    return segments.find((seg) => audioTime >= seg.start && audioTime < seg.end) || null;
  }, [segments, audioTime]);

  const handleSeekToSegment = (segment: Segment) => {
    seekToTimeline(Math.max(0, segment.start), true);
    setControlsVisible(true);
  };
