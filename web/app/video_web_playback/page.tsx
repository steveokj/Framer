...
export default function VideoWebPlaybackPage() {
  ...
  }, [metadata, combinedOffset, videoDuration, timelineDuration]);
  ...
  const handleSeekToSegment = (segment: Segment) => {
    ...
    const duration =
      (Number.isFinite(videoRef.current?.duration || 0) && (videoRef.current?.duration as number)) || videoDuration || null;
    const targetVideoTime = mapTimelineToVideo(Math.max(0, segment.start - offset), duration, metadata);
    const targetAudioTime = Math.max(0, segment.start);
