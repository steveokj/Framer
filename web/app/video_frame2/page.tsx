...
export default function VideoFrame2Page() {
  ...
  const isPreAudioRegion = clampedCurrentTime + combinedOffset <= 0;
  ...
  const seekToTimeline = useCallback(
    (targetTimelineSeconds: number, autoplay = false) => {
      ...
      if (desiredAudio <= 0) {
        ...
        if (Math.abs(audio.currentTime - clampedAudio) > 0.05) {
          try {
      ...
      if (Number.isFinite(timelineSeconds)) {
        ...
        if (audioNode) {
          ...
          if (desiredAudio <= 0) {
            ...
            pendingAudioPlayRef.current = false;
            const clampedAudio = Math.max(0, Math.min(desiredAudio, audioCap ?? desiredAudio));
            if (Math.abs(audioNode.currentTime - clampedAudio) > 0.05) {
              try {
            ...
            if ((shouldPlayAudioRef.current && audioNode.paused) || pendingAudioPlayRef.current) {
              audioNode
                .play()
                .then(() => {
                  pendingAudioPlayRef.current = false;
  ...
  const handleSeekToSegment = (segment: Segment) => {
    seekToTimeline(segment.start, true);
    setControlsVisible(true);
  };
            ...
            <>
              ...
              {isPreAudioRegion && (
                <div style={{ color: "#f97316", fontSize: 12, marginTop: 4 }}>
                  Video is ahead of the audio. Sound will engage once the synced region starts.
                </div>
              )}
              </div>
            </>
