...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const args: string[] = [path.join(process.cwd(), "scripts", "window_clips.py"), "--video", videoPath];

  const screenpipeDbInput = (body?.screenpipeDb as string | undefined)?.trim() || process.env.SCREENPIPE_DB_PATH;
  if (screenpipeDbInput) {
    const resolvedDb = await resolvePath(screenpipeDbInput);
    args.push("--screenpipe-db", resolvedDb);
  }

  const { stdout, stderr, code } = await runPython(args);
  if (code !== 0) {
    const message = stderr || stdout || "Failed to compute window clips";
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "window clip script did not return valid JSON" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
