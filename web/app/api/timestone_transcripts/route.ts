...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const args: string[] = [path.join(process.cwd(), "scripts", "timestone_transcripts.py"), "--db", dbPath];
  if (body?.sessionId) {
    args.push("--session-id", String(body.sessionId));
  }
  if (Number.isFinite(body?.startMs)) {
    args.push("--start-ms", String(body.startMs));
  }
  if (Number.isFinite(body?.endMs)) {
    args.push("--end-ms", String(body.endMs));
  }

  const { stdout, stderr, code } = await runPython(args);
  if (code !== 0) {
    const message = stderr || stdout || "Failed to load transcripts";
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "transcript script did not return valid JSON" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
