...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const args: string[] = [path.join(process.cwd(), "scripts", "timestone_event_frames_index.py"), "--db", dbPath];
  ...
  if (code !== 0) {
    const message = stderr || stdout || "Failed to load event frame index";
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "event frame index script did not return valid JSON" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
