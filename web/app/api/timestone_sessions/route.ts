...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const args: string[] = [path.join(process.cwd(), "scripts", "timestone_events.py"), "--db", dbPath, "--list-sessions"];

  const { stdout, stderr, code } = await runPython(args);
  if (code !== 0) {
    const message = stderr || stdout || "Failed to list timestone sessions";
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "timestone sessions script did not return valid JSON" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
