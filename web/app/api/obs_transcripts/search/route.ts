...
export async function POST(req: NextRequest) {
  ...
  const limit = Number.isFinite(body?.limit) ? Math.max(1, Math.floor(body.limit)) : 200;
  const dbPath = await resolveDbPath(body?.dbPath);
  const root = await resolveRepoRoot();
  const scriptPath = path.join(root, "tools", "scripts", "timestone_transcripts_query.py");
  const args: string[] = [scriptPath, "--db", dbPath, "--mode", "search", "--query", query, "--limit", String(limit)];
  if (model) {
  ...
  if (code !== 0) {
    const message = stderr || stdout || "Failed to search transcripts";
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "search script did not return valid JSON" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
