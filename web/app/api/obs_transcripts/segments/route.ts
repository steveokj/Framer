...
export async function POST(req: NextRequest) {
  ...
  const runId = Number.isFinite(body?.runId) ? Math.floor(body.runId) : 0;
  const dbPath = await resolveDbPath(body?.dbPath);
  const root = await resolveRepoRoot();
  const scriptPath = path.join(root, "tools", "scripts", "timestone_transcripts_query.py");
  const args: string[] = [scriptPath, "--db", dbPath, "--mode", "list", "--video", videoPath];
  if (model) {
  ...
  if (code !== 0) {
    const message = stderr || stdout || "Failed to load transcript segments";
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "segments script did not return valid JSON" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
