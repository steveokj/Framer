...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const args: string[] = [path.join(process.cwd(), "scripts", "timestone_event_ocr.py"), "--db", dbPath];
  if (body?.eventId != null) {
    args.push("--event-id", String(body.eventId));
  }
  if (body?.framePath) {
    args.push("--frame-path", String(body.framePath));
  }

  const { stdout, stderr, code } = await runPython(args);
  if (code !== 0) {
    const message = stderr || stdout || "Failed to load OCR";
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "ocr script did not return valid JSON" }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
