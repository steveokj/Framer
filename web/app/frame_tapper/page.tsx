...
function ChevronLeftIcon({ size = 20, color = "#f8fafc" }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function ChevronRightIcon({ size = 20, color = "#f8fafc" }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const LAST_FOLDER_STORAGE_KEY = "timestone:lastObsFolder:frame_tapper";
const LAST_SESSION_STORAGE_KEY = "timestone:lastSessionId:frame_tapper";
...
export default function LiveEventsOnePage() {
  ...
  const fetchEventFrames = useCallback(
  ...
  }, [filteredEvents, selectedEvent, selectEvent]);
  ...
  const activeFrame = useMemo(() => {
    ...
    return eventFrames[clamped] ?? null;
  ...
  const currentWallMs = useMemo(() => {
    ...
    if (selectedEvent) {
      return selectedEvent.ts_wall_ms;
    }
    return null;
  }, [activeSegment, currentTime, selectedEvent, videoModalOpen]);
  ...
  const openVideoAtWallMs = useCallback((wallMs: number) => {
    setPendingVideoWallMs(wallMs);
    setVideoModalOpen(true);
  }, []);

  useEffect(() => {
    if (!videoModalOpen || pendingVideoWallMs == null) {
      return;
    }
    handleSeekToWallMs(pendingVideoWallMs);
    setPendingVideoWallMs(null);
  }, [handleSeekToWallMs, pendingVideoWallMs, videoModalOpen]);
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft") {
        ...
        event.preventDefault();
        const frame = eventFrames[frameIndex];
        openVideoAtWallMs(frame?.frame_wall_ms ?? selectedEvent.ts_wall_ms);
  ...
  }, [eventFrames, frameIndex, handleNextFrame, handlePrevFrame, openVideoAtWallMs, selectedEvent, videoModalOpen]);
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: "60%",
                        borderRadius: 12,
                        background: "linear-gradient(120deg, rgba(30, 41, 59, 0.4), rgba(30, 41, 59, 0.2), rgba(30, 41, 59, 0.4))",
