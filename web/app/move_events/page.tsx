...
export default function LiveEventsPage() {
  ...
  const triggerOverlay = useCallback(async (payload: any) => {
    try {
      ...
      setOverlayStatus("Sending overlay...");
      const res = await fetch("/api/timestone_overlay", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        ...
        const message = typeof data?.error === "string" ? data.error : "Overlay failed.";
        setOverlayError(message);
        setOverlayStatus(null);
        return;
      }
      setOverlayStatus("Overlay shown.");
      setTimeout(() => setOverlayStatus(null), 2000);
    } catch {
      setOverlayError("Overlay failed.");
      setOverlayStatus(null);
    }
  }, []);
