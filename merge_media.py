#!/usr/bin/env python3
"""
Utility for merging a Screenpipe frame video with a separately recorded audio track.

The tool rebuilds the video timeline using the actual capture timestamps so playback
occurs in real time, then muxes the aligned audio into a single MP4.
"""
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Merge Screenpipe video frames with recorded audio."
    )
    ...
    frame_src.add_argument("--frame-tsv", type=Path, help="TSV export with frame metadata (id, offset_index, timestamp, ...).")
    frame_src.add_argument(
        "--screenpipe-db",
        type=Path,
        help="Path to Screenpipe's SQLite database (frames/video_chunks tables).",
    )
...
def load_frames_from_tsv(tsv_path: Path, video_path: Path) -> List[FrameStamp]:
    frames: List[FrameStamp] = []
    with tsv_path.open("r", encoding="utf-8") as fh:
        header = fh.readline().strip().split("\t")
        try:
            idx_offset = header.index("offset_index")
            idx_timestamp = header.index("timestamp")
            idx_name = header.index("name")
        except ValueError as exc:  # pragma: no cover - defensive
            raise SystemExit(f"frame TSV is missing required columns: {exc}") from exc

        for raw in fh:
            cols = raw.rstrip("\n").split("\t")
            if len(cols) <= max(idx_offset, idx_timestamp, idx_name):
                continue
            if Path(cols[idx_name]) != video_path:
                continue
            try:
                offset = int(cols[idx_offset])
                stamp = datetime.fromisoformat(cols[idx_timestamp])
            except ValueError:
                continue
            frames.append(FrameStamp(offset, stamp))

    if not frames:
        raise SystemExit(f"No frames found in {tsv_path} for {video_path}")

    frames.sort(key=lambda f: f.offset_index)
    return frames
...
def load_frames_from_db(db_path: Path, video_path: Path) -> List[FrameStamp]:
    ...
    try:
        ...
        if row is None:
            raise SystemExit(f"Video chunk {video_path} not present in {db_path}")
        chunk_id = row["id"]
        rows = conn.execute(
    ...
    if not frames:
        raise SystemExit(f"No frame rows found for chunk id {chunk_id}")
    return frames
...
def load_audio_metadata(db_path: Optional[Path], audio_path: Path) -> Optional[AudioMeta]:
    ...
    try:
        rel = audio_path.relative_to(Path.cwd())
        candidates.add(str(rel))
    except ValueError:
        pass
    normalized = set()
    for cand in candidates:
        normalized.add(cand.replace("/", "\\"))
        normalized.add(cand.replace("\\", "/"))
    ...
    try:
        ...
        for cand in normalized:
            ...
            if row:
                break
        if row is None:
            # Fallback: match on basename if direct match failed
            row = conn.execute(
...
def write_concat_manifest(
    ...
    assert len(frames) == len(stamps), "Frame count mismatch between media and timestamps"
    lengths: List[float] = []
    ...
    with manifest_path.open("w", encoding="utf-8") as fh:
        ...
        # Repeat last file for concat demuxer requirement
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.frame_tsv:
        frame_stamps = load_frames_from_tsv(args.frame_tsv.resolve(), video_path)
    else:
        frame_stamps = load_frames_from_db(args.screenpipe_db.resolve(), video_path)

    # Extract frames just once
    with tempfile.TemporaryDirectory(prefix="merge_media_") as tmpdir:
        ...
        print(f"Merging {len(frames)} frames over ~{total_duration:.2f}s with audio session {audio_meta.session_id}", file=sys.stderr)
        if audio_filter:
